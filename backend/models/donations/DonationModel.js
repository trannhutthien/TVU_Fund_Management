...
const createPublicDonation = async (donationData) => {
  ...
  try {
    ...
    const { ten, email, soDienThoai, soTien, quyId, ghiChu, hinhThuc, loaiNhaTaiTro, toChuc, diaChi, taiKhoanNganHangId, chungTu, maGiaoDich } = donationData;
    ...
    const [insertDonationResult] = await connection.query(
      ...
      [nhaTaiTroId, quyId, soTien, hinhThuc || 'Chuyen khoan', maGiaoDich || null, ghiChu, chungTu || null, taiKhoanNganHangId || null]
    );
...
const createStaffDonation = async ({ nhaTaiTroId, quyId, soTien, hinhThuc, ghiChu, chungTu, hinhAnhMinhChung, taikhoannganhangId }) => {
  ...
  try {
    ...
    const proofDoc = chungTu || hinhAnhMinhChung || null;
