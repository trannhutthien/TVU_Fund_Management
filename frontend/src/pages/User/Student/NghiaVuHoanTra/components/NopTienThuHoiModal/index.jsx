...
const NopTienThuHoiModal = ({ isOpen, conLai, bankInfo, tenQuy, onSubmit, onClose, submitting }) => {
  ...
  if (!isOpen || conLai === undefined || conLai === null) return null;
  ...
  const handleSubmit = async () => {
    ...
    if (soTienNum > conLai) {
      setError(`Số tiền không được vượt quá ${formatCurrency(conLai)}.`);
      return;
    }
    if (!file) {
    ...
    try {
      ...
      console.error('Nop tien thu hoi error:', err);
    ...
    <div className={styles.overlay} onClick={handleClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.header}>
          <h3 className={styles.title}>Nộp tiền thu hồi</h3>
        ...
        <div className={styles.body}>
          <div className={styles.conLaiInfo}>
            <span className={styles.conLaiLabel}>Còn lại phải nộp</span>
            <span className={styles.conLaiAmount}>{formatCurrency(conLai)}</span>
          </div>
            ...
            <div className={styles.bankInfoBox}>
              ...
              {tenQuy && (
                <div className={styles.transferContentBox}>
                  ...
                  <span className={styles.transferContent}>THU HOI {tenQuy}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className={styles.fieldWrap}>
            ...
            {soTien && parseFloat(soTien) > 0 && (
              <span className={styles.hint}>
                Còn lại sau khi nộp: {formatCurrency(conLai - parseFloat(soTien || 0))}
              </span>
            )}
          </div>
          ...
          <div className={styles.fieldWrap}>
            ...
            <div
              className={`${styles.uploadArea} ${file ? styles.uploadAreaHasFile : ''} ${error && !soTien ? styles.uploadAreaError : ''}`}
              onDrop={handleDrop}
        ...
        <div className={styles.footer}>
          ...
          <button
            ...
            disabled={submitting || uploading || !soTien || !file}
          >
...
export default NopTienThuHoiModal;
